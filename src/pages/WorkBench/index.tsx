...
export const WorkBenchPage: FC<any> = (props) => {
  // const { global, actions } = props;

  const calendarBodyRef = React.createRef<any>();
  const [date, setDate] = useState(new Date());
  const [open, setOpen] = useState(false);
  const [calendarBodyDom, setCalendarBodyDom] = useState<any>(null);
  ...
  const notify = (content: any, openProps: any) => {
    setOpen(openProps);
    if (calendarBodyRef.current) {
