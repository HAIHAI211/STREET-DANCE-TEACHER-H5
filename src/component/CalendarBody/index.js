...
export default (props) => {
  ...
  const [weekLabelArray, setWeekLabelArray] = useState([]);
  const selectDateRef = useRef(new Date());
  ...
  // 设置weekList值
  const setWeekListValue = (firstDayOfmonth) => {
    let newWeekList = [];
  ...
  // 日期className显示
  const dateClassNameShow = (dayItem) => {
